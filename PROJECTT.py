from tkinter import *     #access UI function
import tkinter.messagebox  #special function

root = Tk()               #setting up the main loop
root.geometry("1350x750+0+0")  #size
root.title("Tic Tac Toe")      #title
root.configure(background = 'blue')     #backgroundcolor
Tops = Frame(root,bg = 'blue',pady = 2, width =1350, height = 100, relief = RIDGE)
Tops.grid(row=0,column=0)
flag =0

lblTitle = Label(Tops,font=('arial',50,'bold'),text = "Tic Tac Toe",bd = 21,fg='yellow', bg='red',justify = CENTER)
lblTitle.grid(row=0,column=0)    #tic tac toe

MainFrame = Frame(root,bg = 'yellow',pady = 2, width =1350, height = 100, relief = RIDGE)
MainFrame.grid(row=1,column=0)

LeftFrame = Frame(MainFrame ,bd=10, width =750, height=500, pady=2,padx=10,bg='orange'  ,relief=RIDGE)
LeftFrame.pack(side=LEFT)

RightFrame  =  Frame(MainFrame ,bd=10, width =750, height=500, padx=10,pady=2,bg='pink',relief=RIDGE)
...
buttons = StringVar()
click = True
def checker(buttons):      #button click event
    global click
...
def scorekeeper():   #win condition is checked
		...
		if (button1["text"] == "X" and button2["text"] == "X" and button3["text"] == "X"):
			...
			tkinter.messagebox.showinfo("Tic Tac Toe",pa)       #win celebrate
			reset()
...
def Newgame():         #reset score and names
	global pa,pb,Player1,Player2
	reset()
	PlayerX.set(0)
	PlayerO.set(0)
...
# labelling the right frame1 and frame2
...
button3 = Button(LeftFrame, text=" ", font=('arial',30 ,'bold'), height = 3, width=8, bg='white', command=lambda:checker(button3))         #goto buttons
